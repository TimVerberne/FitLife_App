...
- **Removed the Strava integration.** Strava's API requires the developer
  ...
  here, so the "Link account" flow, the sync edge function, and the related
  schema/columns have been taken back out. Nothing else changes — this
  reverts the one prior commit that added it.
